  ...
  return (
    <div className="flex-1 flex flex-col h-full overflow-hidden">
      <header className="px-6 py-4 border-b border-aegis-200 bg-white">
        <h2 className="text-lg font-semibold">{t("modes.title")}</h2>
      ...
      <div className="flex-1 overflow-y-auto px-6 py-6">
        ...
        <div className="max-w-2xl mx-auto mt-6 p-4 rounded-xl bg-aegis-50 border border-aegis-200 text-xs text-aegis-600 leading-relaxed">
          <p className="font-semibold text-aegis-700 mb-1">How cost saving works</p>
          <ul className="list-disc list-inside space-y-1 text-aegis-600">
            <li>
...
function ModeCard({
  ...
  const accentText = accent === "amber" ? "text-amber-600" : "text-blue-600";
  const accentBorder = active
    ? accent === "amber"
      ? "border-amber-400 ring-2 ring-amber-200"
      : "border-blue-400 ring-2 ring-blue-200"
    : "border-aegis-200 hover:border-aegis-300";

  return (
    <button
      onClick={onClick}
      className={`text-left p-5 rounded-2xl border-2 transition-all bg-white ${accentBorder}`}
    >
      ...
      <p className="text-xs text-aegis-600 leading-relaxed mb-3">{desc}</p>
      <div className="text-[11px] text-aegis-500">
        Cost: <span className="font-semibold text-aegis-700">{cost}</span>
      </div>
    </button>
  );
}
