...
pub async fn ai_chat_stream(
    ...
    // Phase 2: stream chunks over Tauri events.
    Err(AegisError::Internal(
        "streaming chat is not yet implemented in v0.1".into(),
    ))
}
...
pub struct ExecCommandParams {
    pub command: String,
    /// If true, skips the safety policy (only set after user confirmation).
    pub authorized: bool,
}
...
pub struct ConfirmActionParams {
    pub token: String,
    /// The action to authorize: "command:<cmd>" | "file:<path>:<content>" | "app:<name>"
    pub action: String,
}
...
pub fn computer_confirm_action(
    _state: State<'_, Arc<Mutex<AppState>>>,
    _params: ConfirmActionParams,
) -> Result<()> {
    // Phase 2: maintain a pending-token table. For v0.1, the frontend re-issues
    // the original request with `authorized=true` after the user confirms.
    Err(AegisError::Internal(
        "token-based confirmation is implemented in Phase 2 — re-call the original \
         command with authorized=true after user consent".into(),
    ))
}
...
pub fn security_quarantine_list(
    ...
    // v0.1: the quarantine store is in the defender task; we don't have
    // a shared handle here. Phase 2 lifts it to AppState.
    let _ = state;
    Vec::new()
}

#[tauri::command]
pub fn security_restore_file(_id: String) -> Result<()> {
    Err(AegisError::Internal(
        "quarantine restore is wired in Phase 2".into(),
    ))
}
