...
pub async fn fetch_quote(symbol: &str) -> Result<FinanceQuote> {
    let client = reqwest::Client::builder()
        ...
        .build()
        .map_err(|e| anyhow!(format!("http client: {e}")))?;
...
async fn fetch_crypto(client: &reqwest::Client, coin_id: &str) -> Result<FinanceQuote> {
    ...
    let resp = client
        ...
        .send()
        .await
        .map_err(|e| anyhow!(format!("coingecko: {e}")))?;
    if !resp.status().is_success() {
        return Err(anyhow!(format!("coingecko returned {}", resp.status())));
    }
    let body = resp.text().await.map_err(|e| anyhow!(format!("coingecko body: {e}")))?;
    let parsed: HashMap<String, HashMap<String, f64>> = serde_json::from_str(&body)
        .map_err(|e| anyhow!(format!("coingecko json: {e}")))?;
    let entry = parsed
        .get(coin_id)
        .ok_or_else(|| anyhow!(format!("coingecko: coin '{}' not in response", coin_id)))?;
    let price = entry
        .get("usd")
        .copied()
        .ok_or_else(|| anyhow::anyhow!("coingecko: missing 'usd' field"))?;
...
async fn fetch_fx(client: &reqwest::Client, pair: &str) -> Result<FinanceQuote> {
    ...
    if pair.len() != 6 {
        return Err(anyhow!(format!("fx pair must be 6 chars, got '{}'", pair)));
    }
    let base = &pair[..3];
    let quote = &pair[3..];
    ...
    let resp = client
        ...
        .send()
        .await
        .map_err(|e| anyhow!(format!("ecb: {e}")))?;
    if !resp.status().is_success() {
        return Err(anyhow!(format!("ecb returned {}", resp.status())));
    }
    let body = resp.text().await.map_err(|e| anyhow!(format!("ecb body: {e}")))?;
    let rates = parse_ecb_rates(&body);
    let price = if base == "EUR" {
        rates.get(quote).copied().ok_or_else(|| anyhow!(format!("ecb: no rate for {}", quote)))?
    } else if quote == "EUR" {
        let r = rates.get(base).copied().ok_or_else(|| anyhow!(format!("ecb: no rate for {}", base)))?;
        1.0 / r
    } else {
        // cross-rate via EUR
        let base_to_eur = rates.get(base).copied().ok_or_else(|| anyhow!(format!("ecb: no rate for {}", base)))?;
        let eur_to_quote = rates.get(quote).copied().ok_or_else(|| anyhow!(format!("ecb: no rate for {}", quote)))?;
...
async fn fetch_stock(client: &reqwest::Client, symbol: &str) -> Result<FinanceQuote> {
    ...
    let resp = client
        ...
        .send()
        .await
        .map_err(|e| anyhow!(format!("stooq: {e}")))?;
    if !resp.status().is_success() {
        return Err(anyhow!(format!("stooq returned {}", resp.status())));
    }
    let body = resp.text().await.map_err(|e| anyhow!(format!("stooq body: {e}")))?;
    let mut lines = body.lines();
    let _header = lines.next();
    let row = lines.next().ok_or_else(|| anyhow::anyhow!("stooq: empty csv"))?;
    let cols: Vec<&str> = row.split(',').collect();
    if cols.len() < 7 {
        return Err(anyhow!(format!("stooq: malformed row: {row}")));
    }
    let close: f64 = cols[6].parse().map_err(|_| anyhow!(format!("stooq: bad close: {}", cols[6])))?;
